...
const PORTAL_URL = process.env.PORTAL_BASE_URL || "https://www.bystudiomada.com.br/portal-mada/";
...
function assertNotificationEncoding(...values) {
  const corrupted = values.find((value) => /\uFFFD|Ã[¡-¿]|Â[¡-¿]|[A-Za-zÀ-ÿ][?]{1,2}[A-Za-zÀ-ÿ]/u.test(String(value || "")));
  if (corrupted) throw new Error("A notificação contém caracteres inválidos e não será enviada até ser corrigida.");
}
...
function buildNotificationEmail(delivery) {
  ...
  assertNotificationEncoding(recipientName, title, message, organizationName);
...
function buildSdrInvitationEmail({ recipientName, recipientEmail, actionLink }) {
  const safeName = String(recipientName || "SDR").trim() || "SDR";
  const safeEmail = String(recipientEmail || "").trim();
  const safeLink = String(actionLink || "").trim();
  if (!safeEmail || !safeLink) throw new Error("Convite sem destinatário ou link de acesso.");
  assertNotificationEncoding(safeName, safeEmail);
  const subject = "Seu acesso ao Portal Comercial Mada";
  const text = [
    `Olá, ${safeName}.`,
    "",
    "Você recebeu acesso ao Portal Comercial Mada.",
    "Use o botão abaixo para criar sua senha. O link é individual e expira por segurança.",
    "",
    `Criar minha senha: ${safeLink}`,
    "",
    "Depois de definir a senha, volte à tela de login do Portal e entre com este e-mail.",
...
<body style="margin:0;padding:0;background:#f4f5f8;color:#17181b;font-family:Arial,Helvetica,sans-serif;">
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="background:#f4f5f8;padding:32px 12px;"><tr><td align="center">
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="max-width:620px;background:#fff;border:1px solid #e3e5ea;border-radius:8px;overflow:hidden;">
      <tr><td style="background:#111214;padding:22px 28px;"><div style="color:#fff;font-size:19px;font-weight:800;letter-spacing:1.5px;">MADA</div><div style="margin-top:4px;color:#aeb2bd;font-size:11px;font-weight:700;letter-spacing:1px;text-transform:uppercase;">Portal Comercial</div></td></tr>
      <tr><td style="padding:34px 28px 30px;"><div style="display:inline-block;padding:7px 10px;border-radius:4px;background:#efedff;color:#5548e7;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;">Novo acesso</div>
        <p style="margin:24px 0 8px;color:#676b76;font-size:14px;line-height:1.6;">Olá, ${escapeHtml(safeName)}.</p>
        <h1 style="margin:0;color:#17181b;font-size:25px;line-height:1.28;font-weight:750;">Seu acesso ao Portal Comercial Mada</h1>
        <p style="margin:16px 0 0;color:#444854;font-size:15px;line-height:1.7;">Você recebeu um convite para trabalhar no CRM da Mada. Crie sua senha pelo botão abaixo. O link é individual e expira por segurança.</p>
        <a href="${escapeHtml(safeLink)}" style="display:inline-block;margin-top:26px;padding:13px 20px;border-radius:5px;background:#6557ff;color:#fff;text-decoration:none;font-size:14px;font-weight:750;">Criar minha senha</a>
        <p style="margin:22px 0 0;color:#676b76;font-size:13px;line-height:1.6;">Depois de definir a senha, volte à tela de login do Portal Mada e entre com <strong>${escapeHtml(safeEmail)}</strong>.</p>
      </td></tr>
      <tr><td style="border-top:1px solid #eceef2;padding:18px 28px;color:#858995;font-size:11px;line-height:1.6;">Este e-mail foi enviado automaticamente pelo Portal Comercial Mada. Nenhum pagamento é realizado por e-mail.</td></tr>
    </table>
  </td></tr></table>
</body></html>`;
  return { subject, html, text };
...
async function sendSdrInvitationEmail({ recipientName, recipientEmail, actionLink }) {
  if (!process.env.RESEND_API_KEY) {
    const error = new Error("RESEND_API_KEY não configurada.");
    error.statusCode = 503;
    throw error;
  }
  const email = buildSdrInvitationEmail({ recipientName, recipientEmail, actionLink });
  const response = await fetch("https://api.resend.com/emails", {
    method: "POST",
    headers: {
      Authorization: `Bearer ${process.env.RESEND_API_KEY}`,
      "Content-Type": "application/json; charset=utf-8",
      "Idempotency-Key": `portal-invitation/${String(recipientEmail).trim().toLowerCase()}`,
    },
    body: JSON.stringify({ from: emailSender(), to: [recipientEmail], subject: email.subject, html: email.html, text: email.text, tags: [{ name: "source", value: "portal_mada_invitation" }] }),
  });
...
async function markDeliveryProviderStatus(providerMessageId, status) {
  if (!providerMessageId || !status) return;
  return withClient((client) => client.query(`
    update private.notification_email_deliveries
    set provider_status = $2,
      delivered_at = case when $2 in ('delivered', 'delivery') then now() else delivered_at end,
      bounced_at = case when $2 in ('bounced', 'complained', 'failed') then now() else bounced_at end,
      updated_at = now()
...
module.exports = {
  assertNotificationEncoding,
  buildNotificationEmail,
  buildSdrInvitationEmail,
  dispatchPortalNotificationEmails,
  emailSender,
  markDeliveryProviderStatus,
  sendSdrInvitationEmail,
