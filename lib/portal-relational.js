...
async function syncPayments(client, before, after, session) {
  ...
  for (const item of after || []) {
    ...
    const values = [
      id, session.user.organizationId, item.contractId, Number(item.amountCents || 0), item.type || "contract_payment",
    ...
    if (!previous) {
      ...
      const result = await client.query(`
        update public.customer_payments set amount_cents=$4, payment_type=$5, method=$6,
        ...
        where id=$1 and organization_id=$2 and contract_id=$3 and version=$16
      `, [...values, Number(item._version || 1)]);
