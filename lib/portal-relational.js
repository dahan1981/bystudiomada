...
const { applyMutationSet, mergeStateForUser } = require("./portal-scope");
...
async function readRelationalState(client, session) {
  ...
  const [approvalRows, conditionRows, contractRows, paymentRows, commissionRows, batchRows, batchItemRows, projectRows, stageRows, meetingRows, fileRows, notificationRows, auditRows] = await Promise.all([
    ...
    queryAll(client, "select * from public.meetings where organization_id = $1 order by starts_at asc", [organizationId]),
    queryAll(client, "select * from public.files where organization_id = $1 order by created_at desc", [organizationId]),
    queryAll(client, `
      select notification.*, coalesce(reads.read_at, notification.read_at) as effective_read_at
      from public.notifications notification
      left join public.notification_reads reads on reads.notification_id = notification.id and reads.user_id = $3
      where notification.organization_id = $1
        and ($2::boolean or notification.recipient_user_id = $3 or notification.recipient_role = $4::portal_member_role)
      order by notification.created_at desc
    `, [organizationId, isManager, session.user.id, session.user.role]),
  ...
  const contracts = contractRows.filter((row) => isManager || accessibleOpportunityIds.has(row.opportunity_id) || (row.payload?.recordMode === "external" && row.payload?.sdrId === session.user.id)).map((row) => ({
  ...
  const meetings = meetingRows.filter((row) => isManager || row.created_by === session.user.id || row.sdr_user_id === session.user.id || (row.manager_user_ids || []).includes(session.user.id)).map((row) => ({
    ...(row.payload || {}), id: row.id, organizationId: row.organization_id, opportunityId: row.opportunity_id,
    createdBy: row.created_by, sdrId: row.sdr_user_id, brandName: row.brand_name, startsAt: iso(row.starts_at), endsAt: iso(row.ends_at),
    managerIds: Array.isArray(row.manager_user_ids) ? row.manager_user_ids : [], status: row.status, meetingLink: row.meeting_link || "",
    context: row.context || "", notes: row.notes || "", createdAt: iso(row.created_at), updatedAt: iso(row.updated_at), _version: row.version,
  }));
  ...
  return {
    ...
    notifications: notificationRows.map((row) => ({
      ...
      read: Boolean(row.effective_read_at), readAt: iso(row.effective_read_at), createdAt: iso(row.created_at),
    })),
...
async function syncOpportunities(client, before, after, session) {
  ...
  for (const incoming of after || []) {
    ...
    if (previous && (previous.status === "rejected" || previous.terminalRejection)) {
      throw Object.assign(new Error("Uma oportunidade recusada pelo gestor e imutavel."), { statusCode: 409 });
    }
    ...
    const values = [
      ...
      incoming.nextAction || null, incoming.nextActionDate || null, iso(incoming.archivedAt), JSON.stringify({ ...incoming, id }), session.user.id,
    ];
    if (!previous) {
      await client.query(`
        insert into public.opportunities (
          ...
          next_action, next_action_date, archived_at, payload, created_by, updated_by
      ...
      const result = await client.query(`
        update public.opportunities set
          ...
          payment_plan=$11, next_action=$12, next_action_date=$13, archived_at=$14, payload=$15::jsonb,
...
async function syncBatches(client, before, after, session) {
  ...
  for (const item of after || []) {
    ...
    const id = uuid(item.id);
    const receiptPath = await resolveStoragePath(client, item.receiptAttachmentId);
    const values = [id, session.user.organizationId, item.sdrId, Number(item.sequenceNumber || 1), Number(item.totalAmountCents || 0), item.status || "batched", receiptPath, iso(item.paidAt), item.paidBy || null];
    if (!previous) {
      await client.query(`
      ...
      const result = await client.query(`
        update public.payout_batches set total_amount_cents=$5, status=$6,
          receipt_storage_path=$7, paid_at=$8, paid_by=$9, version=version+1
        where id=$1 and organization_id=$2 and sdr_user_id=$3 and version=$10
      `, [...values, Number(item._version || 1)]);
      ensureVersion(result, "lote de comissão");
    }
...
async function syncMeetings(client, before, after, session) {
  const beforeById = byId(before);
  for (const item of after || []) {
    const previous = beforeById.get(item.id);
    if (previous && !changed(previous, item)) continue;
    const id = uuid(item.id);
    const values = [
      id, session.user.organizationId, item.opportunityId || null, item.createdBy || session.user.id, item.sdrId || session.user.id,
      item.brandName || "Lead", iso(item.startsAt), iso(item.endsAt), JSON.stringify(item.managerIds || []), item.status || "scheduled",
      item.meetingLink || null, item.context || "", item.notes || "", JSON.stringify({ ...item, id }),
    ];
    if (!previous) {
      await client.query(`
        insert into public.meetings (
          id, organization_id, opportunity_id, created_by, sdr_user_id, brand_name,
          starts_at, ends_at, manager_user_ids, status, meeting_link, context, notes, payload
        ) values ($1,$2,$3,$4,$5,$6,$7,$8,$9::jsonb,$10,$11,$12,$13,$14::jsonb)
      `, values);
    } else {
      const result = await client.query(`
        update public.meetings set opportunity_id=$3, sdr_user_id=$5, brand_name=$6,
          starts_at=$7, ends_at=$8, manager_user_ids=$9::jsonb, status=$10, meeting_link=$11,
          context=$12, notes=$13, payload=$14::jsonb, version=version+1, updated_at=now()
        where id=$1 and organization_id=$2 and version=$15
      `, [...values, Number(item._version || 1)]);
      ensureVersion(result, "reunião");
    }
  }
}
...
async function writeRelationalState(client, incoming, session, mutations = null) {
  const current = await readRelationalState(client, session);
  const mutated = applyMutationSet(current, incoming, mutations);
  const merged = mergeStateForUser(current, mutated, session.user);
  await client.query("begin");
  try {
    await syncOpportunities(client, current.opportunities, merged.opportunities, session);
    await syncMeetings(client, current.meetings, merged.meetings, session);
