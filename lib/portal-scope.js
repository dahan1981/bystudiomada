...
function visibleStateForUser(source, user) {
  ...
  const meetings = data.meetings.filter((item) => item.createdBy === user.id || item.sdrId === user.id || (item.managerIds || []).includes(user.id));
  const contracts = data.contracts.filter((item) => opportunityIds.has(item.opportunityId) || (item.recordMode === "external" && item.sdrId === user.id));
...
function mergeStateForUser(source, incomingSource, user) {
  ...
  const ownedMeetings = incoming.meetings.filter((item) => item.createdBy === user.id || item.sdrId === user.id);
  return {
    ...current,
    opportunities: mergeSdrOpportunities(current.opportunities, incomingOwnedOpportunities, user.id),
    meetings: mergeOwnedCollection(current.meetings, ownedMeetings, (item) => item.createdBy === user.id || item.sdrId === user.id),
