const { publicUser } = require("./portal-security");
...
function preserveCredentials(incomingUsers, currentUsers) {
...
const sdrOpportunityTransitions = {
  ...
  rejected: new Set(["rejected", "cancelled"]),
...
function sanitizeSdrOpportunities(currentItems, incomingItems, userId) {
  ...
  return (incomingItems || []).filter((item) => item.sdrId === userId).map((item) => {
...
function mergeStateForUser(source, incomingSource, user) {
  ...
  if (user.role === "admin_manager") {
    return {
      ...incoming,
      users: preserveCredentials(incoming.users, current.users),
    };
  }
  ...
  return {
    ...current,
    opportunities: mergeOwnedCollection(current.opportunities, incomingOwnedOpportunities, ownedOpportunity),
